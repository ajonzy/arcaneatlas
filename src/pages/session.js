...
export default function Session() {
    ...
    const socketInitializer = async () => {
        if (typeof window !== "undefined") {
            ...
            socket.on("moveToken", (id, token) => {
                ...
                else stateRef.current.sessionMap.pieces.tokens[id] = token
                ...
                const revealSquares = (selectedX, selectedY) => {
                    ...
                    const blockingWalls = walls.filter(wall => !["window", "hidden-window discovered", "door opened", "hidden-door discovered opened"].some(type => stateRef.current.sessionMap.pieces[wall.key.split("-").slice(2).join("-")]?.includes(type)))
                    ...
                    squares.forEach(square => {
                        ...
                        if (
                            ...
                            revealedWalls.forEach(wall => stateRef.current.sessionMap.pieces[wall.key.split("-").slice(2).join("-")] = `${stateRef.current.sessionMap.pieces[wall.key.split("-").slice(2).join("-")]} revealed`)
                        }
                    })
                }
    ...
    const updateSession = async () => {
        const sessionId = user.sessions.find(session => session.name === stateRef.current.sessionName).id
        ...
        .then(data => { if (data.status !== 200) console.log(data.message) })
        .catch(error => console.log("Error updating session:", error))
    }

    return (
        <div className='page-wrapper session-wrapper'>
            {userType === "gm" ? <GMTools socket={socket} /> : <PlayerTools />}
            {sessionMap &&
                <Grid 
